...
// TODO:
// figure out the peeking through walls stuff, key line is the step_size calculation. (int)ing overall_buf makes it work but not without it
...
public:
    ...
    vector2 ray_end;
    int ray_len;
    int ray_num{672}; // Number of rays for castin
    int fov{90};
    float player_speed;
...
bool Player::rayInWall(vector2 point, char grid[COLS + 1][ROWS + 1], bool x_chosen, bool y_chosen)
{
    ...
    {
        // std::cout << (int)(point.y / TILE_SIZE) - col_mod_y << " " << (int)(point.x / TILE_SIZE) - col_mod_x << "HERE" << std::endl;
...
void Player::castRay(char grid[COLS + 1][ROWS + 1])
{
    ...
    for (int i = 0; i < ray_num; i++)
    {
        ...
        {
            step_size_y = fabs((player_posn.y) - (((int)(player_posn.y / TILE_SIZE) + step_mod_y) * TILE_SIZE)); // value is greater than 32 in 3rd quadrant?
        }
        ...
        {
            // std::cout << step_size_y << " " << step_size_x << std::endl;
            if (!x_chosen)
            {
                if (started_x)
            ...
            // std::cout << overall_buf.y << " " << overall_buf.x << std::endl;
...
void Player::castWalls(int playerHeight, int wallHeight, double sightAngle) // min height of wall is 20 pixels
{
    ...
    double proj_plane_distance = (ROWS * TILE_SIZE / (2 * tan((fov / 2) * M_PI / 180))) * TILE_SIZE; // Multiplied by TILE_SIZE for scaling
